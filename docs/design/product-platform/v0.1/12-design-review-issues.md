...
- **问题**：恢复接口使用笼统的「Account 范围恢复权限」「平台范围恢复权限」，但各对象恢复策略不同：Session「管理员不能恢复他人」（11 §66）、私有 Skill「不能恢复他人」（10 §59）、Account 共享 Resource/Skill 可恢复（09/10）、User 恢复走回收站（05 §12.6 无专用接口）。
...
- **证据**：设计（11 §65.1）写「默认保留最近 3 个逻辑 Turn、6000 Token 预算」；源码中 3/6000 是 VikingBot 插件默认值（`bot/vikingbot/hooks/builtins/openviking_hooks.py:198-201`），引擎兜底为 3 Turn / 12000 Token（`openviking/session/session.py:1724-1730`）。
...
| 15 | 10/11 文档编号 | 10 号文档占 §50-65，11 号文档从 §60 开始，§60-65 两篇重叠 |
...
| # | 处置 | 修订人 | 日期 |
| --- | --- | --- | --- |
| 1 | 保持现状：不新增标题字段、不展示标题；11 文档 §64.2 已修订 | 助手 | 2026-08-18 |
