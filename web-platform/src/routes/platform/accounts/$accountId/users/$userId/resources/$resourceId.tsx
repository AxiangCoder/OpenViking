 ...
 * 平台级成员私有 Resource 只读预览（13 §89.3 同 84.2，05 §12.6，P4-E4 AC②⑦）。
 *
...
export default function PlatformAccountUserResourcePage() {
  ...
  const load = useCallback(() => {
    ...
    fetchPlatformMemberResource(accountId, userId, resourceId)
      .then(setDetail)
      .catch((error) => {
        setLoadError({
  ...
  if (invalidId) {
    return (
      <div className="card" data-testid="platform-member-resource-invalid-id">
        <h2>未找到该 Resource</h2>
  ...
  return (
    <div className="admin-page" data-testid="platform-member-resource-page">
      <PlatformSubjectDataBanner actorLabel={actorLabel} accountLabel={accountLabel} subjectLabel={subjectLabel} />
      <div className="profile-actions">
        <Link to={`/platform/accounts/${accountId}/users/${userId}/data`} className="placeholder-back" data-testid="platform-member-resource-back">
          ← 返回成员数据
        </Link>
      </div>

      {loadError ? (
        <div className="admin-load-error" role="alert" data-testid="platform-member-resource-load-error">
          ...
          <div className="profile-actions">
            <button type="button" onClick={load} data-testid="platform-member-resource-retry">
              重试
            </button>
          </div>
        </div>
      ...
      {detail != null ? (
        <section className="card" data-testid="platform-member-resource-detail">
          <div className="admin-page-header">
            <h2 data-testid="platform-member-resource-name">{detail.name}</h2>
          ...
          <dl className="profile-row">
            ...
            <dt>内容</dt>
            <dd data-testid="platform-member-resource-content">
            ...
            <dt>自动同步</dt>
            <dd data-testid="platform-member-resource-watch">
          ...
          {detail.overview ? (
            <div className="card" data-testid="platform-member-resource-overview">
              <h3>概览</h3>
              <pre className="skill-markdown">{detail.overview}</pre>
            </div>
          ) : null}
          <p className="profile-hint" data-testid="platform-member-resource-readonly-note">
