/**
 * 平台代管共享 Resource 新增弹窗（09 §40 同款契约，05 §12.6 平台表，P4-E4 AC⑨）。
 *
 * - 入口固定归属（09 §40.1）：目标 Account 共享 Resource，无归属下拉切换；
 * - 来源：本地文件（每文件一个 Resource）/ 公开 HTTPS 网页 / 公开 HTTPS Git
 *   （09 §40.2）；批量按文件独立成败（09 §40.5）；
 * - 上传限制取自 capabilities（AC②：max_files_per_batch / max_file_size_bytes，
 *   前端只做浏览器预检，服务端仍独立强制）；
 * - 标签严格 key=value（AC④）；URL 禁 userinfo/私网（09 §40.6）；
 * - 上传/一次性 URL 导入完成后不可 Watch；稳定来源在详情页配置自动同步
 *   （09 §40.6：含 Query 的 URL 只做一次性导入；v0.1 平台代管不提供 Watch
 *   配置入口，同步沿用导入时的配置）。
 */
...
import {
  platformImportResources,
  platformUploadResourceFile,
  type PlatformImportItemInput,
} from "@/features/iam/platform";

export type ImportSourceKind = "upload" | "web" | "git";

export interface PlatformImportDialogProps {
  accountId: string;
  targetLabel: string;
  onClose: () => void;
...
function newIdempotencyKey(): string {
  ...
  if (cryptoObj && typeof cryptoObj.randomUUID === "function") {
    return `platform-resource-import-${cryptoObj.randomUUID()}`;
  }
  return `platform-resource-import-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 14)}`;
}
...
export default function PlatformImportDialog({
  ...
  useEffect(() => {
    fetchCapabilities()
      ...
      .catch((error) =>
        setFormError(
          resourceErrorMessage(error, "无法获取上传能力配置，请刷新后重试（限制取自服务端）。"),
        ),
      );
  }, []);
    ...
    try {
      ...
      const entries = await Promise.all(
        files.map(async (entry): Promise<FileEntry> => {
          ...
          try {
            const uploaded = await platformUploadResourceFile(accountId, entry.file);
      ...
      const submitted: PlatformImportItemInput[] = [
        ...entries
      ...
      const batch = await platformImportResources(
        accountId,
        submitted.map((item) => ({
          ...item,
  ...
  return (
    <div className="confirm-dialog-backdrop" role="dialog" aria-modal="true" aria-label="新增 Resource">
      <div className="confirm-dialog import-dialog">
        <h3>新增 Resource</h3>
        <p className="profile-hint" data-testid="platform-import-target">
          保存到：<strong>{targetLabel}</strong>（入口固定归属，无归属切换；Platform Super Admin
          按目标 Account 代管共享 Resource，09 §40.1）
        </p>

        {formError ? (
          <p className="login-error" data-testid="platform-import-error">
            {formError}
          </p>
        ) : null}
          ...
          <div className="import-upload-area">
            ...
            {capabilities ? (
              <p className="profile-hint">
                ...
                （取自 Capabilities）；每个文件生成一个 Resource，上传文件不能开启自动同步（09 §40.5）。
              </p>
            ) : null}
            {files.length > 0 ? (
        ...
        {resultItems ? (
          <div className="import-result" data-testid="platform-import-result">
            <p>
              {allAccepted
                ? "导入请求已提交，正在处理。"
